...
- 외부 수집은 서버가 임의 URL을 탐색하는 기능이 아니다. 관리자가 요청한 allowlist 공급자의 단일 HTTPS URL만 수집하고, redirect·DNS 재검증으로 사설망/비표준 포트 접근을 차단한다.
  결과는 불변 스냅샷과 검수 후보로 보관하며 관리자 명시 승인 전 공용 카탈로그·RAG에 반영하지 않는다.
- V18/V19는 위 계약의 요청·스냅샷 영속 기반까지만 구현했다. 실제 네트워크 수집, URL 보안 검증, 사이트 파서와 관리자 승인 API는 공식 API 또는 서면 허가를 받은 데이터 소스가 정해질 때 구현하며
  기본 비활성 상태로 유지한다.
- AI 문서 초안은 사용자 소유의 기존 자기소개서만 대상으로 한다. 공고·현재 자기소개서 버전·선택 이력서의 입력 스냅샷을 보존하고, AI 실패에는 fallback 문서를 만들지 않는다. 사용자가 검수 후 명시
  적용할 때만 새 자기소개서 버전과 RAG UPSERT를 만든다. 상세 계약은 [외부 수집·AI 문서 초안 계약](COLLECTION_AND_DRAFT.md)을 따른다.

- 관리자 사용자 관리는 목록·상세·역할과 `ACTIVE`·`SUSPENDED` 상태 변경 및 hard delete를 제공한다. 모든 관리자 변경은 관리자 행을 ID순 잠그고 호출자가 여전히 활성 관리자인지
  재확인한다.
- 자기 강등·자기 정지·자기 강제 삭제를 금지한다. 활성 관리자의 회원 탈퇴와 다른 관리자 정지·강등·삭제도 같은 관리자 잠금을 사용해 최소 한 명의 활성 관리자를 보존한다.
- 정지는 Refresh Token을 폐기하고 로그인·재발급을 막는다. JWT 요청마다 DB 상태를 확인해 이미 발급된 Access Token도 즉시 403으로 차단하며 삭제된 사용자의 기존 토큰은 401로
  처리한다.
- 강제 삭제와 회원 탈퇴는 공통 삭제 서비스를 사용해 개인 문서 RAG DELETE, DB cascade, 이력서 파일의 커밋 후 정리 계약을 공유한다.
- 관리자 API에 passwordHash·providerId를 공개하지 않는다. RAG 운영 API는 후속 구현이며 개인 문서 본문을 노출하지 않는 방향으로 설계한다.

- 면접 결과는 인증 사용자 소유의 `COMPLETED` 세션에만 제공하고 타인 세션은 404, 미완료 세션은 409로 처리한다. 완료 평가는 점수 통계에 포함하며 대기·처리·실패·미요청은 개수와
  `PENDING`/`PARTIAL`/`COMPLETED` 분석 상태로 구분한다.
- 성장 분석은 완료 시각 기준 기본 90일·최대 365일이며 직무 스냅샷·기업·공고 필터를 지원한다. 기간 전체 평균은 답변 수 가중, 변화량은 최근 최대 3개와 직전 최대 3개 세션 평균의
  동일 비중 비교다.
- 자유 텍스트 강점·개선사항은 통계 키워드로 합산하지 않는다. 질문 유형과 STAR·논리성·직무 적합성 조합의 표본 3개 이상만 강점·약점 후보로 삼고 80점 미만 약점에 결정적 학습
  로드맵을 제공한다. MVP 성장 분석은 추가 AI 호출이나 분석 결과 영속화를 사용하지 않는다.

- 답변은 질문당 한 번 제출하며 앞뒤 공백 제거 후 Java String 길이 1~10,000이다. 같은 내용 재전송은 기존 결과를 반환하고 다른 내용은 409다. 새로운 답변/꼬리 질문은
  IN_PROGRESS에서만 가능하며 완료 후 기존 결과 재조회는 허용한다.
- 답변 기반 꼬리 질문은 초기 질문당 최대 하나이며 추가 깊이는 금지한다. 순번은 기존 질문 뒤에 붙이고 부모 ID로 연결한다. 꼬리 질문에 대한 답변은 저장할 수 있다. 모든 질문에 답해야 한다는 완료 조건은
  추가하지 않았다.
- 꼬리 질문은 별도 동기 POST(200)로 생성한다. 직무 스냅샷·부모 질문·답변만 사용하며 추가 RAG 검색은 없다. FALLBACK_ONLY는 답변 인용 정형 질문이고 AI 실패는 답변 보존·503·수동
  재요청으로 처리한다. 자동 작업/재시도 및 외부 호출 exactly-once는 보장하지 않는다.
- DB 트랜잭션 밖에서 생성 후 소유권·상태·중복을 다시 검사한다. 세션 잠금과 답변·꼬리 질문·최대 순번의 잠금 읽기를 조합하여 반복 읽기의 오래된 read view 영향을 차단한다. context는 내부
  보존하며 API에는 노출하지 않는다.
- 답변·꼬리 질문은 2026-09-15 전체 753개 자동 회귀 검증을 완료했고 2026-09-16 실제 Chat 연동과 AI 평가 smoke도 성공했다. 결과·성장 분석은 같은 날 전체 803개 회귀를
  완료했다.
...
- 세션별 V12 작업의 DB 선점·120초 lease·attempt UUID 및 원자적 완료로 한 세션의 질문 묶음 중복 저장을 막는다. 외부 호출 exactly-once와 생성 POST 재전송의 멱등성은 보장하지
  않는다.
- 빈 RAG 결과 또는 재시도 가능한 장애의 3회 소진은 고정 질문 fallback으로 READY 처리한다. fallback은 질문 출처와 작업 사유로 구분하며 실패 코드로 성공을 표현하지 않는다.
- 자동 재시도 대기 중에는 GENERATING을 유지하고 인증·설정·quota·내부 오류는 FAILED로 처리한다. DB 오류는 전파한다. 수동 재시도는 소유자·FAILED·질문 없음 조건에서 최대 2회이고
  endpoint는 아직 없다.
- AI 입력은 세션 스냅샷을 우선하며 현재 RAG 자료와 함께 제한된 JSON으로 저장한다. 자동 검증은 mock 모델과 MySQL 기반이며, 2026-09-16 실제 OpenAI
  Chat·Qdrant RAG smoke를 별도로 완료했다.
- 설정 테스트는 호스트 환경변수·JVM 속성을 격리한다. 운영 설정 자체는 변경하지 않으며 기본 worker는 비활성, 기본 모드는 FALLBACK_ONLY다.

- 면접 내부 RAG 검색은 세션에 저장된 기업·공고·선택 자기소개서·선택 이력서의 정확한 `(sourceType, sourceId)`만 허용한다. Qdrant metadata filter 이후에도 허용 집합, 활성
  generation과 현재 원본 존재·소유권을 다시 검사한다.
- 세션의 기업 ID는 V11에서 원본 FK 없이 저장한다. 개인 문서 미선택은 해당 검색 범위 생략이며, 검색 결과 없음은 정상적인 빈 목록으로 처리한다. Vector Store 장애와 손상 metadata는 내부
  검색에서 숨기지 않고 질문 생성 계층으로 전파한다.
...
- 회원 탈퇴는 회원 행을 먼저 잠그고 자기소개서·이력서를 ID 순서로 잠근 뒤 각 원본의 RAG DELETE를 같은 트랜잭션에서 모두 등록한다. 이후 회원 삭제의 DB cascade와 이력서 파일의 커밋 후 정리를
  수행하며, 등록 실패 시 회원·문서·파일을 유지한다. 관리 중인 문서가 삭제 사용자 엔티티를 참조하는 JPA flush 오류를 피하기 위해 RAG 변경을 먼저 flush하고 사용자 ID 일괄 삭제로
  cascade를 실행한다.
- 관리자 RAG 조회는 원본 키·순번·상태·시도 횟수·실패 코드·시각만 반환하며 제목·본문·revision·저장 키·소유자 ID·attempt ID는 노출하지 않는다.
- 실패 작업 재시도는 기존 작업 ID·원본 순번·스냅샷과 누적 attempt를 보존하고 실행 기회 3회를 추가한다. 수동 재시도는 최대 2회이며 최신 순번이 아닌 UPSERT는 차단하고 DELETE는 이후 순번이
  있어도 기존 순번 이하 정리를 위해 허용한다.
- 현재 원본 재색인은 원본 변경과 같은 잠금 순서로 최신 상태를 읽고 새 원본 순번의 UPSERT를 등록한다. 이력서 추출 본문이 준비되지 않았거나 원본이 삭제된 경우 새 작업을 만들지 않는다.
