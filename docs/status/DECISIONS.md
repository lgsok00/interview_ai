...
- 관리자 사용자 관리(2026-09-17)는 목록·상세·역할 변경을 우선 구현했다. 실제 DB 역할을 검사하고 역할 변경 시 관리자 잠금 조회 결과에 호출자가 남아 있는지 재확인한다. 서비스·HTTP 29개,
  MySQL 통합 4개와 전체 836개 회귀가 성공했다.
- 역할 변경의 자기 강등 금지는 마지막 관리자 검사보다 우선한다. 현재 회원 탈퇴는 마지막 관리자 탈퇴를 막지 않으므로 시스템 전체의 관리자 최소 1명 보존을 보장한다고 해석하지 않는다. 관리자 탈퇴 정책은 후속
  확인 사항이다.
