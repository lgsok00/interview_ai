...
- V20 `cover_letter_drafts`는 사용자·대상 자기소개서, 재생성 원본, 자기소개서 현재 버전과 기업·공고·이력서 입력 스냅샷, SHA-256 입력 해시, prompt/model, 생성·적용 결과를
  저장한다.
- 스냅샷 조립은 대상 자기소개서를 잠그고 현재 버전을 읽으며, 명시 이력서는 소유권과 추출 완료 본문을 검사한다. 생략 시 대표 이력서를 사용하고 미설정·미준비는
  `REPRESENTATIVE_RESUME_NOT_READY`로 처리한다.
- 구조화 Chat 출력은 `title`, `content`, `changeSummary`, `warnings`만 허용하고 길이·형식·추가 필드를 검증한다. 입력 문서는 신뢰할 수 없는 데이터로 전달하며 사실 창작과
  외부 도구 호출을 금지한다.
