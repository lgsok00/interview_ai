...
6. RAG 기반 구축
    ...
    - 다음 단계: 새 migration으로 작업 본문·상태 저장과 순번 발급을 같은 트랜잭션에 연결하고 원본 변경·삭제와 등록의 원자성 및 스냅샷 조회 시점 설계
    - 후속 단계: worker 선점·lease·활성 generation·삭제 tombstone. 현재 행 잠금은 등록 기반이며 worker 동시 실행 제어는 미구현
