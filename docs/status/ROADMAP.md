...
6. RAG 기반 구축
    ...
    - V7·JPA 작업 등록 영속화 검증 완료: UPSERT 스냅샷·DELETE 키·초기 PENDING 저장과 순번 발급의 동시 커밋·롤백. 신규 46개 포함 전체 428개 성공, 실패·오류·건너뜀 0 (2026-09-07)
    - 다음 단계: 원본 변경·삭제와 등록의 원자성, 스냅샷 조회 시점·원본 동시 변경 잠금 정책을 정하고 기존 CRUD에 등록 서비스 연결
    - 후속 단계: 실행 상태 전이 저장·worker 선점·lease·활성 generation·삭제 tombstone. 현재 행 잠금은 등록 기반이며 worker 동시 실행 제어는 미구현
