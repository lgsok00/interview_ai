...
- 기업·공고는 로그인 사용자가 조회하는 공용 데이터다. 관리자가 수동 등록·수정·삭제하며, 외부 수집은 관리자 요청의 단일 HTTPS URL에서 불변 후보를 만든 뒤 명시 승인으로만 반영한다. 자동
  병합·공개·삭제와 사이트 전체 크롤링은 하지 않는다. 상세는 [외부 수집·AI 문서 초안 계약](COLLECTION_AND_DRAFT.md)을 따른다.
- V18/V19 외부 수집 요청·스냅샷 영속 기반은 검증했지만 실제 수집과 승인 API는 연결하지 않았다. 현재 채용공고 등록은 기존 관리자 직접 입력을 유지한다.
