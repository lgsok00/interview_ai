...
초안은 V20 입력 스냅샷, 구조화 Chat 생성·lease/재시도 worker, 생성·조회·재생성·명시 적용 API까지 구현했고 2026-09-21 기본 자동 검증을 완료했다.
생성기·설정·scheduler·MySQL 동시성 직접 테스트와 실제 Chat 호출은 대기다. 외부 수집은 공용 기업·채용공고의 **관리자 검수 후보**를 만들고, AI 문서 초안은 인증 사용자의 **자기소개서 검수본
...
3. V20 사용자 소유 draft·입력 스냅샷·SHA-256 입력 해시와 Chat 어댑터·worker를 구현했다. 120초 lease, 최대 3회 자동 시도, 만료 작업 회수, attempt UUID와 늦은 완료
   차단을 포함한다.
4. 소유자 생성·목록·상세·재생성·적용 API와 비활성 AI 실패 등록, 기준 버전 충돌·멱등 적용을 구현하고 초안 25개·전체 976개 자동 회귀를 완료했다. 실제 Chat과 실행 심화 테스트는 대기다.
5. 정상: 수집 후보 승인 후 공용 데이터/RAG 반영, AI 초안 생성·검수 수정 적용. 경계: 빈 선택값, 최대 길이, redirect, 만료 lease, 재전송, 기준 버전 일치. 실패: SSRF 대상,
