...
2026-09-17에 확정한 구현 전 계약이다. 이 문서는 API나 DB가 이미 구현되었다는 뜻이 아니다. 외부 수집은 공용 기업·채용공고의 **관리자 검수 후보**를 만들고, AI 문서 초안은 인증 사용자의 *
*자기소개서 검수본**을 만든다. 둘 다 원본 데이터를 자동으로 바꾸지 않는다.
...
1. V18으로 수집 요청·불변 스냅샷·상태/락 제약을 추가하고, URL 검증과 HTTP 수집 포트를 테스트 가능한 인터페이스로 분리한다.
2. 관리자 요청·조회·재시도·거절과 단일 승인 반영을 구현한다. 승인 시 기존 카탈로그 쓰기와 RAG 등록을 재사용한다.
3. V19로 사용자 소유 draft·입력 스냅샷·실행 작업을 추가하고, Chat 어댑터·worker·조회·재생성·적용을 구현한다.
