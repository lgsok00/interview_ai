...
2026-09-17에 확정한 계약이다. 2026-09-18 V18/V19로 외부 수집 요청·불변 스냅샷의 영속 기반을 구현·검증했으며, 실제 네트워크 수집·사이트 파서·관리자 승인 API는 보류했다. AI 문서
초안은 V20 입력 스냅샷, 구조화 Chat 생성·lease/재시도 worker, 생성·조회·재생성·명시 적용 API까지 구현했고 2026-09-21 생성기·설정·scheduler·MySQL 동시성 선택 검증과 실제
OpenAI Chat smoke를 완료했다. 외부 수집은 공용 기업·채용공고의 **관리자 검수 후보**를 만들고, AI 문서 초안은 인증 사용자의 **자기소개서 검수본
**을 만든다. 둘 다 원본 데이터를 자동으로 바꾸지 않는다.
...
3. V20 사용자 소유 draft·입력 스냅샷·SHA-256 입력 해시와 Chat 어댑터·worker를 구현했다. 120초 lease, 최대 3회 자동 시도, 만료 작업 회수, attempt UUID와 늦은 완료
   차단을 포함한다.
4. 소유자 생성·목록·상세·재생성·적용 API와 비활성 AI 실패 등록, 기준 버전 충돌·멱등 적용을 구현하고 기본 초안 25개·전체 976개 자동 회귀를 완료했다.
5. 생성기·설정·scheduler·실제 MySQL 선점/lease/동시성 24개 선택 테스트와 실제 OpenAI Chat 초안 생성 smoke를 완료했다. 신규 테스트를 포함한 전체 회귀도 112개
   클래스·1,000개 성공했다.
6. 정상: 수집 후보 승인 후 공용 데이터/RAG 반영, AI 초안 생성·검수 수정 적용. 경계: 빈 선택값, 최대 길이, redirect, 만료 lease, 재전송, 기준 버전 일치. 실패: SSRF 대상,
