...
초안은 V20 입력 스냅샷과 구조화 Chat 생성·lease/재시도 worker 기반까지 구현했으나 컴파일·자동 테스트와 HTTP 검수/재생성·적용은 대기다. 외부 수집은 공용 기업·채용공고의 **관리자 검수 후보
**를 만들고, AI 문서 초안은 인증 사용자의 **자기소개서 검수본**을 만든다. 둘 다 원본 데이터를 자동으로 바꾸지 않는다.
...
3. V20 사용자 소유 draft·입력 스냅샷·SHA-256 입력 해시와 Chat 어댑터·worker를 구현했다. 120초 lease, 최대 3회 자동 시도, 만료 작업 회수, attempt UUID와 늦은 완료
   차단을 포함하며 컴파일·자동 테스트는 대기다.
4. 소유자 생성·목록·상세·재생성·적용 API와 비활성 AI 실패 등록, 기준 버전 충돌·멱등 적용을 구현한다.
