...
- 최초 선택 실행의 통합 4개 실패는 JVM 서울 시각과 DB UTC 시각이 다른 fixture 문제였고 DB `UTC_TIMESTAMP(6)` 기준으로 수정한 뒤 4개 클래스·24개가 모두 성공했다.
- smoke 전용 자기소개서·추출 완료 이력서 seed SQL과 PowerShell 5.1 호환 ASCII smoke 스크립트를 추가했다.
- 실제 OpenAI 호출에서 draft 2가 1회 시도로 `REVIEW_READY`가 되어 구조화 결과 저장을 확인했다.
- 신규 테스트를 포함한 전체 회귀는 6분 9초 동안 112개 클래스·1,000개가 성공했고 실패·오류·건너뜀은 0이다.
- 확인한 HEAD는 `91145c8`이며 심화 테스트·smoke 스크립트·문서는 작업 트리에서 커밋 대기다.

## 2026-09-21 — AI 자기소개서 초안 HTTP 흐름 기본 자동 검증 성공

- 소유자 생성·목록·상세·재생성·명시 적용 API를 추가하고 비활성 AI 실패 저장, 최신 입력 재스냅샷, 기준 버전 충돌, 검수 수정 적용과 동일 재전송 멱등성을 연결했다.
- 엔티티·worker·서비스·컨트롤러 4개 클래스·25개 선택 테스트와 전체 108개 클래스·976개 회귀가 성공했고 실패·오류·건너뜀은 0이다.
- 생성기·설정·scheduler·실제 MySQL 실행 동시성 직접 테스트와 실제 OpenAI Chat smoke는 후속 검증으로 남긴다.
- 현재 HEAD는 `4de9a0e`이며 HTTP 구현·테스트·문서는 작업 트리에서 커밋 대기다.

## 2026-09-18 — AI 자기소개서 초안 영속·생성 실행 기반 구현, 검증 대기

- V20으로 사용자·대상 자기소개서와 기업·공고·이력서 입력 스냅샷, 입력 해시, 생성·적용 결과와 실행 상태를 저장하는 `cover_letter_drafts`를 추가했다.
- 현재 자기소개서 버전과 추출 완료 이력서를 스냅샷하고, 구조화 Chat 결과 검증과 prompt injection 데이터 격리 정책을 구현했다.
- 120초 lease, 최대 3회 자동 시도, 만료 작업 회수, attempt UUID 기반 늦은 완료 차단과 조건부 scheduler를 추가했다.
- 생성·조회·재생성·명시 적용 HTTP API와 자동 테스트는 후속 작업이다. 사용자 컴파일·테스트 결과가 없어 구현됨·검증 대기로 기록한다.
- 현재 HEAD는 `a8174ad`이며 V20 기반과 문서는 작업 트리에서 커밋 대기다.

## 2026-09-18 — 외부 수집 영속 기반 전체 회귀 성공

- V18/V19로 외부 수집 요청, 불변 성공·실패 스냅샷, 상태/lease, 수동 재시도 2회와 최대 9회 이력 제약을 추가했다.
- 단위·MySQL 통합 17개와 전체 XML 104개·951개가 성공했고 실패·오류·건너뜀은 0이다.
- 실제 네트워크 수집·사이트 파서·관리자 승인 API는 공식 API 또는 허가된 데이터 소스가 정해질 때까지 보류하며 현재 공고 등록은 관리자 직접 입력을 유지한다.
- 현재 HEAD는 `082262e`이며 실패 코드 선검증 수정·테스트·문서는 작업 트리에서 커밋 대기다. 다음 작업은 V20 AI 자기소개서 초안이다.

## 2026-09-17 — 관리자 사용자 정지·강제 삭제 전체 회귀 성공

- V17 사용자 상태와 관리자 상태 변경·강제 삭제 API를 추가하고 로컬·OAuth2·Refresh Token·기존 Access Token에서 정지 사용자를 차단했다.
- 관리자 행 잠금으로 자기 정지·자기 강제 삭제와 마지막 활성 관리자의 강등·정지·탈퇴를 막았다. 회원 탈퇴와 강제 삭제는 개인 문서 RAG·파일 정리 공통 서비스를 사용한다.
- 선택 XML 20개·161개, `AdminRagIntegrationTest` 21개, 전체 XML 101개·934개가 성공했고 실패·오류·건너뜀은 0이다.
- 현재 HEAD는 `944661e`이며 구현·테스트·문서는 작업 트리에서 커밋 대기다. 다음 작업은 외부 수집·AI 문서 초안 계약이다.

## 2026-09-17 — 관리자 RAG·회원 탈퇴 보완 전체 회귀 성공

- 관리자 원본·작업 조회, 기존 작업 재시도, 현재 원본 새 순번 재색인과 V16 수동 재시도 횟수를 구현했다. 운영 응답에서는 개인 문서 내용과 내부 식별 정보를 제외한다.
- 회원 탈퇴는 사용자 선행 잠금과 개인 문서 잠금을 유지하고 RAG 변경 flush 후 ID 일괄 삭제로 DB cascade를 실행하도록 보완했다.
- 선택 실행 98개가 32초에 성공했고 전체 XML 100개·916개가 4분 45초에 성공했다. 실패·오류·건너뜀은 0이다.
- 현재 HEAD는 `79851f7`이며 구현·테스트·문서는 작업 트리에서 커밋 대기다. 다음 작업은 관리자 사용자 정지·강제 삭제 계약이다.

## 2026-09-17 — 관리자 RAG 80개 성공·회원 탈퇴 잠금 누락 확인

- 선택 재실행 XML 98개 중 94개 성공·4개 실패를 확인했다. 관리자 RAG 80개는 모두 성공했다.
- 회원 탈퇴의 flush·ID 일괄 삭제는 반영됐지만 사용자 선행 잠금·존재 검사가 누락되어 단위 테스트 4개가 실패했다. 호출 복원안을 제공했으며 전체 회귀는 대기다.

## 2026-09-17 — 관리자 RAG 구현·선택 실행 실패 분석

- 원본·작업 운영 조회, 같은 순번·스냅샷을 유지하는 실패 재시도, 현재 원본의 새 순번 재색인과 V16 수동 재시도 횟수를 구현했다. 응답은 개인 문서 제목·본문·revision을 제외한다.
- 사용자 선택 실행 79개 중 73개 성공·6개 실패를 확인했다. 테스트의 트랜잭션 프록시 stubbing 문제와 연쇄 오염을 수정했다.
- 문서가 영속성 컨텍스트에 로드된 상태에서 사용자 JPA 삭제가 실패하는 경합 사례를 확인했다. 실제 회원 탈퇴 경로의 회귀 테스트와 flush 후 ID 일괄 삭제 수정안을 준비했다. 애플리케이션 반영·재검증은
  대기다.
- 현재 HEAD는 `79851f7`이며 새 구현과 테스트는 작업 트리에 있다. 변경 전 전체 836개 성공을 현재 변경의 검증 결과로 사용하지 않는다.
