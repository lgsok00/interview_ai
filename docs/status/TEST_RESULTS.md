...
- 사용자 실행: `frontend`에서 `npm test` — Vitest 2개 파일·6개 성공.
- 사용자 실행: `npm run lint` — 경고·오류 0. `npm run build` — `tsc -b && vite build` 성공.
- 토큰 저장소 2개와 공통 client 4개 테스트가 성공했다. 백엔드 전체 회귀의 최신 실행은 아래 2026-09-21 기록이며 이번 실행에 포함되지 않는다.
- Codex는 테스트를 직접 실행하지 않고 사용자 출력과 실제 저장소 변경사항을 확인했다.
...
- 사용자 선택 실행: `.\gradlew.bat test --tests "com.interviewai.coverletter.draft.*"` — BUILD SUCCESSFUL (37초).
- 선택 XML 4개·25개 성공, 실패·오류·건너뜀 0: 엔티티 6개, worker 5개, 서비스 8개, 컨트롤러 6개다.
- 사용자 전체 실행: `.\gradlew.bat test` — BUILD SUCCESSFUL (5분 48초). XML 108개·976개 성공, 실패·오류·건너뜀 0.
- OpenJDK class-data sharing 경고는 결과에 영향을 주지 않았다. 생성기 JSON 파싱·설정·scheduler·실제 MySQL 선점/동시성과 실제 OpenAI Chat 호출은 직접 검증하지
  않았다.

### 외부 수집 V18/V19 영속 기반 선택 및 전체 회귀 성공 (2026-09-18)

- 사용자 선택 실행: `.\gradlew.bat test --tests "com.interviewai.externalcollection.*"` — BUILD SUCCESSFUL (27초).
- 선택 XML 3개·17개 성공, 실패·오류·건너뜀 0: 요청 상태 6개, 스냅샷 6개, 실제 MySQL 통합 5개다.
- 최초 선택 실행은 17개 중 JSON 문자열 공백 비교 1개가 실패했다. MySQL JSON 정규화 동작에 맞춰 JSON 구조 비교로 수정했다.
- 다음 실행은 `@DataJpaTest`에 `ObjectMapper` Bean이 없어 통합 테스트 5개가 생성 실패했다. 테스트 전용 mapper를 사용하도록 수정한 뒤 선택 실행이 성공했다.
- 사용자 전체 실행: `.\gradlew.bat test` — BUILD SUCCESSFUL (5분 35초). XML 104개·951개 성공, 실패·오류·건너뜀 0.
- OpenJDK class-data sharing 경고는 결과에 영향을 주지 않았다. Codex는 테스트를 실행하지 않고 사용자 출력·최신 XML·실제 변경사항을 확인했다.
...
- 사용자 전체 실행: `.\gradlew.bat test` — BUILD SUCCESSFUL (4분 10초).
- 최신 전체 XML 84개에서 753개 성공, 실패·오류·건너뜀 0. 면접 XML 19개·181개, 신규 5개 클래스·49개 성공이다. 선택 실행 XML은 전체 실행으로 갱신됐으므로 XML 수치는 전체 실행
  근거다.
- V13·답변 길이·소유권·멱등 재전송·깊이·완료 후 저장 차단, AI 출력/timeout·interrupt, DB rollback·cascade 및 동시 답변/질문 저장·순번 할당을 검증했다.
- 답변·부모별 꼬리 질문·마지막 순번 조회의 PESSIMISTIC_WRITE 반영을 확인했다. 반복 읽기의 기존 read view를 가진 두 트랜잭션에서도 중복 저장과 순번 충돌을 방지하는 테스트가 성공했다.
- 실제 Chat 네트워크 호출은 수행하지 않았다. OpenJDK class-data sharing 경고는 결과에 영향을 주지 않았다. Codex는 테스트를 실행하지 않고 사용자 출력과 실제 XML을 확인했다.

### 면접 조회·진행·질문 제공·수동 재시도 API 검증 (2026-09-15)

- 사용자 면접 선택 실행: `.\gradlew.bat test --tests "com.interviewai.interview.*"` — BUILD SUCCESSFUL (1분 3초).
- 사용자 전체 실행: `.\gradlew.bat test` — BUILD SUCCESSFUL (3분 50초).
- 전체 실행 XML 79개에서 704개 성공, 실패 0, 오류 0, 건너뜀 0을 확인했다. 면접 XML 14개는 132개 성공이다.
- 소유자별 목록·상세와 안정적 최신순, 질문 순서·허용 상태·내부 context 비노출, 비관적 잠금 시작·완료, 상태 충돌, 소유권 은닉, JWT 필수와 수동 재시도 HTTP 202를 검증했다.
- OpenJDK class-data sharing 경고는 결과에 영향을 주지 않았다. Codex는 테스트를 직접 실행하지 않고 사용자 출력과 실제 XML을 확인했다.
...
- 채용공고 필수 선택, 인증 사용자의 대표 자기소개서 현재 버전과 추출 완료 대표 이력서 자동 선택, 대표 미설정 생략 및 생성 시점 스냅샷 저장을 구현했다.
- 준비되지 않은 대표 이력서의 HTTP 409, 없는 공고·현재 자기소개서 버전·스냅샷 조립 실패, 요청 validation과 인증 실패를 포함한 14개 테스트를 추가했다.
- 사용자 선택 실행: `./gradlew test --tests "com.interviewai.interview.*"` — BUILD SUCCESSFUL (21초).
- 사용자 전체 실행: `./gradlew test` — BUILD SUCCESSFUL (2분 15초).
- 전체 실행 결과의 XML 70개에서 603개 성공, 실패 0, 오류 0, 건너뜀 0을 확인했다. 면접 XML 6개는 39개 성공, 실패·오류·건너뜀 0이다.
- OpenJDK class-data sharing 경고는 테스트 결과에 영향을 주지 않았다. Codex는 테스트를 직접 실행하지 않고 사용자 출력과 XML을 확인했다.

### 면접 세션·질문 영속 기반 검증 (2026-09-12)

- V10 면접 세션·질문 schema, 생성 시점 원본 스냅샷, 세션 상태 전이와 실패·재시도, 질문 순서·유형·생성 출처·RAG context 저장 모델을 추가했다.
- Codex가 엔티티 단위 테스트 19개와 MySQL 통합 테스트 6개를 추가했다.
- 사용자 선택 실행: `./gradlew test --tests "com.interviewai.interview.*"` — BUILD SUCCESSFUL (27초).
- 사용자 전체 실행: `./gradlew test` — BUILD SUCCESSFUL (2분 15초).
- 전체 실행 결과의 XML 67개에서 589개 성공, 실패 0, 오류 0, 건너뜀 0을 확인했다. 면접 XML 3개는 25개 성공, 실패·오류·건너뜀 0이다.
- OpenJDK class-data sharing 경고는 테스트 결과에 영향을 주지 않았다.
