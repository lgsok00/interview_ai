...
1. Chat Model 질문 생성, fallback, 실패·재시도와 중복 생성 방지 정책을 연결한다.
2. 면접 세션 조회·시작·완료와 질문 제공 API를 구현한다.
...
- 브랜치: `main`
- 이번 확인한 HEAD: `e10233f feat: 면접 세션 생성과 대표 문서 스냅샷 조립 추가`
- `main`과 `origin/main`은 같은 커밋을 가리킨다. 면접 세션 전용 내부 RAG 검색 구현·테스트·문서는 작업 트리에서 커밋 대기이며, RAG·면접 선택 304개와 프로젝트 전체 618개 회귀 검증을
  완료했다.
