...
| 영역                   | 상태                                                                                                          |
|----------------------|-------------------------------------------------------------------------------------------------------------|
| 인증·회원 관리             | 구현·검증 완료: Access Token 응답, HttpOnly Refresh Token cookie, CORS, Google·GitHub redirect 로그인, 회원 관리           |
...
| AI 자기소개서 초안          | 구현·선택 검증 완료: V20·입력 스냅샷·Chat 생성·worker·API·생성기/설정/scheduler·MySQL 동시성·실제 Chat smoke                         |
| React 프론트엔드 기반       | Vite·React·TypeScript, 공통 API client·메모리 Access Token·cookie 요청·오류 변환 구현 및 필수 테스트 6개 검증 완료. 인증 상태·화면은 다음 작업 |

## 다음 작업

1. React 프론트엔드의 로그인·토큰 재발급·로그아웃과 OAuth2 callback 흐름, 인증 상태 관리를 구현한다.
2. 인증 상태를 이용해 보호 라우팅을 연결한다.
3. 외부 실제 수집·관리자 승인 API는 공식 API 또는 수집 허가를 받은 데이터 소스가 정해질 때 재개한다.
4. STT/TTS·실시간 면접·PDF 리포트는 MVP 이후 확장 범위다.
...
연결은
보류했다. AI 자기소개서 초안은 V20과 입력 스냅샷·구조화 Chat 생성·비동기 실행, 사용자 생성·조회·재생성·명시 적용, 생성기·설정·scheduler·MySQL 실행 심화 검증과 실제
OpenAI Chat smoke와 새 테스트를 포함한 프로젝트 전체 회귀까지 완료했다.

## 최신 검증

- 실행일: 2026-09-23 — 프론트엔드 공통 API client 필수 검증 성공.
- 사용자 실행: `frontend`에서 `npm test` — Vitest 2개 파일·6개 성공. `npm run lint` — 경고·오류 0. `npm run build` — TypeScript·Vite 빌드
  성공.
- 메모리 토큰 저장·삭제와 빈 값 거부, 인증/공개 요청의 cookie·Bearer 헤더, JSON 본문, 백엔드 오류 변환, body/json 중복 입력 차단을 확인했다. 자동 재발급·OAuth2
  callback·보호 라우팅은 아직 구현·검증하지 않았다.
- 아래 2026-09-21의 1,003개 성공은 별도로 실행한 백엔드 전체 회귀 기록이다.

- 실행일: 2026-09-21 — 브라우저용 인증·CORS 계약 변경 전체 회귀 성공.
- 사용자 전체 실행: `.\gradlew.bat test` — BUILD SUCCESSFUL (5분 47초). 실제 XML 112개 클래스·1,003개 성공, 실패·오류·건너뜀 0.
- 로그인·재발급 응답의 Access Token 전용 JSON, Refresh Token HttpOnly cookie 발급·회전·삭제, OAuth2 성공 redirect와 cookie, credential CORS
  정책을 검증했다.
- OpenJDK class-data sharing 경고는 테스트 결과에 영향을 주지 않았다.

- 실행일: 2026-09-21 — AI 자기소개서 초안 실행 심화 선택 검증과 실제 OpenAI Chat smoke 성공.
- 사용자 선택 실행: 생성기·설정·scheduler·MySQL 통합 테스트 4개 클래스 — BUILD SUCCESSFUL (31초). 실제 XML 24개 성공, 실패·오류·건너뜀 0.
- 생성기 입력 보존·구조화 JSON 파싱·호출 제한, 안전한 설정과 외부 환경 격리, scheduler 큐 소진·한도·예외·interrupt, MySQL V20·선점·lease·늦은 쓰기·동시성·rollback을
  검증했다.
- 최초 MySQL 실행은 JVM 서울 시각과 DB UTC 시각이 다른 fixture 때문에 24개 중 4개가 실패했다. fixture를 `UTC_TIMESTAMP(6)` 기준으로 수정한 뒤 24개가 모두 성공했다.
- 사용자 실제 smoke: `cover-letter-draft-smoke.ps1` — PASS. draft 2가 1회 시도로 `REVIEW_READY`, 기준 버전 1과 구조화 생성 결과 저장을 확인했다.
- 사용자 전체 실행: `.gradlew.bat test` — BUILD SUCCESSFUL (6분 9초). 실제 XML 112개 클래스·1,000개 성공, 실패·오류·건너뜀 0.

- 실행일: 2026-09-21 — AI 자기소개서 초안 HTTP 흐름과 기본 실행 회귀 성공.
- 사용자 선택 실행: `.\gradlew.bat test --tests "com.interviewai.coverletter.draft.*"` — BUILD SUCCESSFUL (37초). XML 4개·25개 성공,
  실패·오류·건너뜀 0.
- 사용자 전체 실행: `.\gradlew.bat test` — BUILD SUCCESSFUL (5분 48초). XML 108개·976개 성공, 실패·오류·건너뜀 0.
- 상태 전이·lease 경계·늦은 응답, worker 오류/interrupt, 비활성 AI 실패, 재생성, 기준 버전 충돌, RAG UPSERT, 적용 멱등성, HTTP 202·validation·인증을 검증했다.
...
- 브랜치: `main`
- 이번 확인한 HEAD: `8bde4c2 feat: React 프론트 기반과 쿠키 인증 연동 구성`
- 프론트엔드 공통 client·테스트·이번 문서 갱신은 작업 트리에서 커밋 대기다. 사용자 실행 Vitest 6개, lint, build가 성공했다. 백엔드 최신 전체 회귀는 2026-09-21의 1,003개
  성공이다.
