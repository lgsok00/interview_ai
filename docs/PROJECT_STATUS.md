...
1. 생성된 면접 세션의 기업·채용공고·자기소개서·이력서 원본 ID로 검색 범위를 제한하는 내부 RAG 계약을 추가한다.
2. Chat Model 질문 생성, fallback, 실패·재시도와 중복 생성 방지 정책을 연결한다.
3. 면접 세션 조회·시작·완료와 질문 제공 API를 구현한다.
...
- 브랜치: `main`
- 이번 확인한 HEAD: `66c152d feat: 면접 세션과 질문 영속 기반 추가`
- `main`과 `origin/main`은 같은 커밋을 가리킨다. 면접 세션 생성 API·대표 문서 스냅샷 조립과 테스트·문서는 작업 트리에서 커밋 대기이며, 면접 선택 39개와 프로젝트 전체 603개 회귀 검증을
  완료했다.
