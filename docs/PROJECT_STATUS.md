...
| React 프론트엔드 기반       | Vite·React·TypeScript 프로젝트, API URL 환경변수와 health 호출 준비 완료. 인증 API client·상태 관리는 다음 작업             |

## 다음 작업

1. React 프론트엔드의 공통 API client를 구성한다. Access Token은 메모리에 두고 모든 인증 쿠키 요청에 `credentials: "include"`를 적용한다.
2. 로그인·토큰 재발급·로그아웃과 OAuth2 callback 흐름을 구현한 뒤 보호 라우팅을 연결한다.
...
- 브랜치: `main`
- 이번 확인한 HEAD: `91145c8 feat(cover-letter): AI 초안 검수·적용 API 추가`
- AI 자기소개서 초안 심화 테스트·smoke 스크립트·검증 문서가 작업 트리에서 커밋 대기다. 신규 24개 선택 검증과 최신 전체 112개 클래스·1,000개 회귀가 성공했다.
